...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    const poll = window.setInterval(async () => {
      const response = await fetch(`${API_URL}/v1/projects/${project.id}`);
  ...
  async function createProject(event) {
    ...
    setCreating(true);
    try {
      const response = await fetch(`${API_URL}/v1/projects`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ title: title || "Untitled recording", text, voice_id: voiceId, speech_rate: "normal" }) });
